...
/*   Updated: 2022/12/22 22:22:53 by rteles-f         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if ((i + 1) < info->height_y && is_in_str(info->map[i + 1][j], "/DB"))
	{
		info->collectables[1] += (info->map[i + 1][j] == 'B');
		info->map[i + 1][j] += 1;
		check_reach(info, (i + 1), j);
	}
	if ((i - 1) >= 0 && is_in_str(info->map[i - 1][j], "/DB"))
	{
		info->collectables[1] += (info->map[i - 1][j] == 'B');
		info->map[i - 1][j] += 1;
		check_reach(info, (i - 1), j);
	}
	if ((j + 1) < info->width_x && is_in_str(info->map[i][j + 1], "/DB"))
	{
		info->collectables[1] += (info->map[i][j + 1] == 'B');
		info->map[i][j + 1] += 1;
		check_reach(info, i, (j + 1));
	}
	if ((j - 1) >= 0 && is_in_str(info->map[i][j - 1], "/DB"))
	{
		info->collectables[1] += (info->map[i][j - 1] == 'B');
		info->map[i][j - 1] += 1;
		check_reach(info, i, (j - 1));
...
// void	check_reach(t_infomap *info, int i, int j)
// {
// 	if ((i + 1) < info->height_y && is_in_str(info->map[i + 1][j], "0CE"))
// 	{
// 		info->collectables[1] += (info->map[i + 1][j] == 'C');
// 		info->map[i + 1][j] -= 1;
// 		check_reach(info, (i + 1), j);
// 	}
// 	if ((i - 1) >= 0 && is_in_str(info->map[i - 1][j], "0CE"))
// 	{
// 		info->collectables[1] += (info->map[i - 1][j] == 'C');
// 		info->map[i - 1][j] -= 1;
// 		check_reach(info, (i - 1), j);
// 	}
// 	if ((j + 1) < info->width_x && is_in_str(info->map[i][j + 1], "0CE"))
// 	{
// 		info->collectables[1] += (info->map[i][j + 1] == 'C');
// 		info->map[i][j + 1] -= 1;
// 		check_reach(info, i, (j + 1));
// 	}
// 	if ((j - 1) >= 0 && is_in_str(info->map[i][j - 1], "0CE"))
// 	{
// 		info->collectables[1] += (info->map[i][j - 1] == 'C');
// 		info->map[i][j - 1] -= 1;
// 		check_reach(info, i, (j - 1));
